...
- `.github/workflows/mixture-parity.yml` evaluates three result families on real human/mouse mixtures against the construction truth manifest: the `xenofilx` selection, Picard `SetNmMdAndUqTags` NM semantics with and without `IS_BISULFITE_SEQUENCE`, and the XenofilteR selection driven by each Picard NM variant. `xenofilx` is built from source; Picard comes from the pinned `enva` environment; XenofilteR is installed at a pinned commit (`de0bbe4c...`) from `PeeperLab/XenofilteR` because it is no longer packaged. XenofilteR loads `GenomeInfoDb`, which requires the `GenomeInfoDbData` annotation package; the Bioconda spec `bioconductor-genomeinfodbdata` resolves without placing that R package into the environment library, so `scripts/gate6/install-xenofilter.sh` installs `GenomeInfoDbData` from the pinned Bioconductor release and asserts its version before `R CMD INSTALL`. The RNA cell re-evaluates the Supplementary Table S5 input at threshold 4; the BS cell is the Supplementary Table S6 50% cell evaluated at threshold 6 under bisulfite mode.
- `.github/workflows/note4-bs-gradient.yml` reconstructs Supplementary Table S6, the 0–99% bisulfite composition gradient (43 cells: a pure-mouse control plus 14 non-zero fractions across three replicates). Fixture labels, sizes, and SHA-256 digests come from `benchmark/note4/mixture-cells.json`; the recorded per-cell metrics are in `benchmark/note4/per-cell-expected-s6.json`. `push` and `pull_request` cover a three-cell representative slice; `workflow_dispatch` with `full_sweep: true` covers all 43 cells.
- `.github/workflows/parameter-grid.yml` reconstructs Supplementary Table S7, the 4x3 audit of mismatch threshold `{5,6,7,8}` against unmapped penalty `{8,10,12}` on the matched 60% human mixtures (three replicates, 36 runs). Recorded values are in `benchmark/note4/per-cell-expected-s7.json`.

Metric differences against the recorded values are reported rather than enforced: the comparisons emit warning annotations, record a per-metric `status` in the JSON report, and never fail the job. Only a missing or structurally invalid evidence set fails, because that means the evaluation did not run. The acceptable range is ±0.5 percentage points, with a separate ±2.0 point "large difference" notice; `compare-note4-gradient.py --fail-on-deviation` restores strict enforcement if needed.
